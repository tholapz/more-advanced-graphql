...
const startServer = async () => {
  ...
  const engine = new Engine({
    engineConfig: {
      ...
      logcfg: { level: 'DEBUG', },
      // where are/is the GraphQL API server(s)?
      origins: [ { url: `http://localhost:${PORT}/graphql` } ],
      // proxies to instantiate
      frontends: [ { host: '127.0.0.1', port: 8080, endpoint: '/graphql' } ],
    },
  });

  // this instantiate HTTP server (=> asynchronous operation)
  await engine.start();
  ...
  authenticate(server);
  // add it to the GraphQL server middleware chain
